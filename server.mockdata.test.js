...
describe("🚀 API Testing - Mock Data", () => {
    ...
    it("should fetch sector data by site_id", async () => {
        const response = await request(app).get(`/sectors/MOCK001`);
        expect(response.status).toBe(200);
        expect(response.body).toHaveProperty("count");
        expect(response.body.count).toBeGreaterThan(0);
    });
...
it("should partially update mock data sector", async () => {
    ...
    it("should delete mock data sector", async () => {
        ...
        expect(response.status).toBe(200);
        expect(response.body).toHaveProperty("message", "✅ Data deleted!");
    });
    ...
    // ✅ 16. Tutup koneksi MongoDB setelah semua tes selesai
    afterAll(async () => {
